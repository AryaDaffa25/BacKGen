...
def get_prompt_spc_few_with_explanation(text,st_span,negative_example_list,positive_example_list,prompt_variant=11):
    if prompt_variant == 11: # This is prompt few-shot 1-shot in the paper
        ...
        raise ValueError(f"You choose variant {prompt_variant} but we do not have it in get_prompt_spc_few_with_explanation function. Please define your variant in that function.")

def get_prompt_spc_bk_with_explanation(text,st_span,bk_list,prompt_variant=1):
    # For prompt_variant 2 and so on
    prompt_bk = write_bk(bk_list)
    if prompt_variant == 1: # This is prompt bk-shot in the paper
        ...
        raise ValueError(f"You choose variant {prompt_variant} but we do not have it in get_prompt_spc_bk_with_explanation function. Please define your variant in that function.")